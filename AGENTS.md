...
| `Remove-BucketObject` | `-Bucket`, `-Key`, `-Match`/`-Filter`, `-Drop` (delete bucket dir), `-Force` (skip confirm), `-Recurse`, `-Depth` (limits recursion, default unlimited), `-PassThru`, `-Quiet`, `-WhatIf` (SupportsShouldProcess) |
...
### Remove-BucketObject — Drop Safety (merged from Remove-Bucket)
...
- `Remove-BucketObject` with `-Bucket` alone warns on empty bucket
