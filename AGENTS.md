...
| `Get-BucketObject` | `-Key` (positional 0), `-Bucket` (positional 1, wildcards ok, default "default" if omitted), `-Match` (hashtable, supports $null), `-Filter` (scriptblock with `$_`), `-Recurse` (opt-in, default is no recursion), `-Depth` (limits recursion, default unlimited), `-First`, `-Skip`, `-Funnel` |
